...
function MisionCard({
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white shadow-sm overflow-hidden transition-transform duration-100 active:scale-[0.98]">
      ...
      {esActiva && (
        <>
          {/* Aviso de foto rechazada */}
          {fotoStats && fotoStats.rechazada > 0 && (
            <div className="mx-4 mb-2 px-3 py-2.5 bg-amber-50 border border-amber-200 rounded-xl">
              <p className="text-xs font-semibold text-amber-700">
                ⚠️ Tenés {fotoStats.rechazada === 1 ? 'una foto rechazada' : `${fotoStats.rechazada} fotos rechazadas`}
              </p>
              <p className="text-xs text-amber-600 mt-0.5">
                Entrá a la misión para rehacer {fotoStats.rechazada === 1 ? 'esa foto' : 'esas fotos'}.
              </p>
            </div>
          )}
          <div className="px-4 pb-2">
            ...
            >
              {fotoStats?.rechazada ? 'Retomar misión' : 'Ir a capturar'}
