  ...
  if (pendientes.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <p className="text-sm text-gray-400">Cuando un gondolero solicite unirse, aparecerá acá.</p>
      </div>
    )
  }
    ...
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-gray-50">
          {pendientes.map(s => {
            const nombre = s.gondolero_alias ?? s.gondolero_nombre ?? 'Gondolero'
                ...
                <td className="px-4 py-3.5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {s.gondolero_alias && s.gondolero_nombre && (
                        <p className="text-xs text-gray-400">{s.gondolero_nombre}</p>
                      )}
                    </div>
                  </div>
                </td>
