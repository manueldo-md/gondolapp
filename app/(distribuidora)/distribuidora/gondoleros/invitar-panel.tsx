...
import { generarLinkInvitacion, vincularPorCodigo, confirmarVinculacionPorCodigo } from './invitar-actions'
...
export function InvitarPanel({
  ...
  const handleWhatsApp = () => {
    ...
    const texto = encodeURIComponent(
      `Te invito a unirte a ${distriNombre} en GondolApp. Hacé clic acá: ${link}`
    )
  ...
  const handleBuscarCodigo = () => {
    ...
    startBuscar(async () => {
      const res = await vincularPorCodigo(codigo.trim(), distriId, distriNombre)
      if (res.error) { setCodigoError(res.error); return }
      if (res.gondolero) setGondoleroEncontrado(res.gondolero)
    })
  }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-sm font-semibold text-gray-700">Invitar gondolero por link</h3>
        </div>
        <p className="text-xs text-gray-500 mb-4">
          Generá un link único (válido 7 días) y compartilo con el gondolero. Al abrirlo, se vinculará automáticamente a tu distribuidora.
        </p>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        ...
        {vinculadoOk && (
          <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg mb-3">
            ...
            <p className="text-sm font-medium text-green-700">¡Gondolero vinculado exitosamente!</p>
          </div>
        )}
        ...
        <div className="flex gap-2">
          <input
            type="text"
            value={codigo}
            onChange={e => { setCodigo(e.target.value.toUpperCase()); setCodigoError(null); setGondoleroEncontrado(null); setVinculadoOk(false) }}
            onKeyDown={e => e.key === 'Enter' && handleBuscarCodigo()}
            placeholder="Ej: NARU-7823"
            ...
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-gondo-amber-50 flex items-center justify-center shrink-0">
                <span className="text-gondo-amber-400 font-bold text-sm">
                  {(gondoleroEncontrado.alias ?? gondoleroEncontrado.nombre ?? '?').charAt(0).toUpperCase()}
                </span>
              </div>
              <div>
