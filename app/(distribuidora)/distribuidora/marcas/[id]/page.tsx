...
import {
  ArrowLeft, TrendingUp, Camera, Package, Clock, RefreshCw,
} from 'lucide-react'
...
export default async function DistriRelacionDetailPage({
  params,
}: {
  params: { id: string }
}) {
  ...
  const solEnviadaPorDistri = solicitudPendiente?.solicitado_por === 'distri' ? 'pendiente' as const
    : ultimaSolicitud?.solicitado_por === 'distri' && ultimaSolicitud?.estado === 'rechazada' ? 'rechazada' as const
    : null
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div>
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{marcaNombre}</h1>
            <p className="text-sm text-gray-500 mt-0.5">Relación con {distriNombre}</p>
          </div>
        ...
        {rel.estado === 'terminada' && (
          <div className="mt-5 pt-4 border-t border-gray-100">
            <ReiniciarRelacionBtnDistri
              ...
              solicitudEstado={solEnviadaPorDistri}
            />
          </div>
        )}
      </div>
      ...
      {campanasActivas.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <CampanasTable campanas={campanasActivas} panel="distribuidora" />
        </div>
      )}
      ...
      {campanasHistorial.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <CampanasTable campanas={campanasHistorial} panel="distribuidora" />
        </div>
      )}
            ...
            {reinicioSols.map((s: any) => (
              <div key={s.id} className="px-5 py-3 flex items-center justify-between text-sm">
                <div>
                  ...
                  <span className="text-xs text-gray-400 ml-2">{new Date(s.created_at).toLocaleDateString('es-AR')}</span>
                </div>
...
function CampanasTable({ campanas, panel }: { campanas: any[]; panel: string }) {
  const base = panel === 'distribuidora' ? '/distribuidora' : '/marca'
  return (
        ...
        {campanas.map((c: any) => {
          ...
          return (
            <tr key={c.id} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="px-4 py-3 text-right">
                <Link href={`${base}/campanas/${c.id}`} className="text-xs text-[#BA7517] hover:underline font-medium">
                  Ver →
                </Link>
              </td>
            </tr>
