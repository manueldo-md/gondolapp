...
import { generarLinkInvitacionDistriRepo } from './actions'
...
export function InvitarRepoDistriPanel({ distriId, distriNombre }: Props) {
  ...
  const handleGenerar = () => {
    ...
    startTransition(async () => {
      const res = await generarLinkInvitacionDistriRepo(distriId, distriNombre)
  ...
  const handleWhatsApp = () => {
    ...
    const msg = encodeURIComponent(
      `Hola! Te invito a vincularte con ${distriNombre} en GondolApp. Aceptá desde este link (válido 7 días): ${link}`
    )
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5">
      ...
      {!link ? (
        <button
          ...
          disabled={isPending}
          className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-[#BA7517] rounded-xl hover:bg-[#9a6012] transition-colors disabled:opacity-50"
        >
