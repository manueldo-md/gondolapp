...
import { actualizarPerfilDistri } from './actions'
...
// Map accent colors to Tailwind classes (must be full strings for Tailwind to include them)
const ACCENT = {
  amber:  { ring: 'focus:ring-gondo-amber-400', btn: 'bg-gondo-amber-400 hover:bg-gondo-amber-500' },
  indigo: { ring: 'focus:ring-gondo-indigo-600', btn: 'bg-gondo-indigo-600 hover:bg-gondo-indigo-700' },
}

export function EditarPerfilDesktopForm({ email, nombre: nombreInicial, celular: celularInicial, accentColor }: Props) {
  ...
  const handleGuardarPerfil = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      await actualizarPerfilDistri({ nombre, celular })
  ...
  const inputClass = `w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 ${accent.ring} text-sm`
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <form onSubmit={handleGuardarPerfil} className="space-y-4">
          ...
          <button type="submit" disabled={guardandoPerfil || !nombre.trim()}
            className={`px-5 py-2.5 ${accent.btn} text-white text-sm font-semibold rounded-lg disabled:opacity-50 transition-colors`}>
