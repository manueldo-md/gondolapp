                ...
                {(stat.tipo === 'seleccion_unica' || stat.tipo === 'seleccion_multiple') && stat.opcionCounts && (
                  <div className="space-y-2">
                    {Object.entries(stat.opcionCounts).sort((a, b) => b[1] - a[1]).map(([op, n]) => {
                      ...
                      return (<div key={op}><div className="flex justify-between text-xs text-gray-600 mb-1"><span>{op}</span><span className="font-semibold">{n} ({pct}%)</span></div><div className="h-2 bg-gray-100 rounded-full overflow-hidden"><div className="h-full bg-gondo-indigo-600 rounded-full" style={{ width: `${pct}%` }} /></div></div>)
                    })}
                  </div>
                )}
