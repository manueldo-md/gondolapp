...
import { generarLinkInvitacionMarca } from './actions'
...
export function InvitarDistriPanel({ marcaId, marcaNombre }: Props) {
  ...
  const handleGenerar = () => {
    startTransition(async () => {
      const res = await generarLinkInvitacionMarca(marcaId, marcaNombre)
      if (res.error) {
        setError(res.error)
      } else if (res.link) {
        setLink(res.link)
  ...
  const handleWhatsApp = () => {
    if (!link) return
    const msg = encodeURIComponent(`Hola! Te invito a vincularte con ${marcaNombre} en GondolApp. Aceptá desde este link (válido por 7 días): ${link}`)
    window.open(`https://wa.me/?text=${msg}`, '_blank')
  }

  return (
    <div className="bg-white rounded-xl border border-gray-200 p-5">
      <h2 className="text-sm font-semibold text-gray-800 mb-1">Invitar distribuidora</h2>
      <p className="text-xs text-gray-500 mb-4">Generá un link de invitación para enviar a una distribuidora. Válido por 7 días.</p>

      {error && (
        <div className="bg-red-50 px-3 py-2 rounded-lg mb-3">
          <p className="text-xs text-red-600">{error}</p>
          <BotonReportarError errorTecnico={error} contexto="marca/distribuidoras — generarLinkInvitacion" />
        </div>
      )}
