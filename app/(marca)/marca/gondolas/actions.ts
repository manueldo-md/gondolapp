...
export async function aprobarFotoMarca(fotoId: string) {
  ...
  await admin.from('fotos').update({ estado: 'aprobada' }).eq('id', fotoId)

  // Incrementar comercios_relevados en la campaña
  ...
  // Incrementar comercios_completados en participaciones
  ...
  // Acreditar puntos si no fueron acreditados todavía
  if (foto.puntos_otorgados === 0) {
    ...
    if (puntos > 0) {
      ...
      await admin.from('fotos').update({ puntos_otorgados: puntos }).eq('id', fotoId)
