...
const ESTADO_COLOR: Record<EstadoFoto, string> = {
  pendiente: 'bg-gray-100 text-gray-600', aprobada: 'bg-green-100 text-green-700',
  rechazada: 'bg-red-100 text-red-700', en_revision: 'bg-blue-100 text-blue-700',
}
...
export default async function RepoCampanaResultadosPage({
  ...
  const progreso = calcularPorcentaje(c.comercios_relevados, limiteComercio ?? 0)
  ...
  const fixerCount = partData.count ?? 0
    ...
    <div>
      ...
      {limiteComercio && limiteComercio > 0 && (
        <div className="bg-white rounded-xl border border-gray-200 p-5 mb-5">
          ...
          <p className="text-xs text-gray-400 text-right">{c.comercios_relevados}/{limiteComercio} comercios ({progreso}%)</p>
        </div>
      )}
                    ...
                    <div className="mt-3">
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-xs border-collapse">
                          <thead><tr className="border-b border-gray-100"><th className="text-left py-1.5 pr-3 text-gray-400 font-medium">Fixer</th><th className="text-left py-1.5 pr-3 text-gray-400 font-medium">Comercio</th><th className="text-left py-1.5 pr-3 text-gray-400 font-medium">Respuesta</th><th className="text-left py-1.5 text-gray-400 font-medium">Fecha</th></tr></thead>
                          <tbody>
                            {detalles.map((row, ri) => (
                              <tr key={ri} className="border-b border-gray-50">
                                <td className="py-1.5 pr-3 text-gray-700 font-medium">{row.alias ?? '—'}</td>
      ...
      {c.tipo === 'precio' && precioMin !== null && (
        <div className="bg-white rounded-xl border border-gray-200 p-5 mb-5">
          ...
          <div className="grid grid-cols-3 gap-3 text-center">
            <div className="bg-green-50 rounded-xl p-3"><p className="text-xl font-bold text-green-700">${precioMin}</p><p className="text-xs text-gray-500 mt-0.5">Mínimo</p></div>
            <div className="bg-blue-50 rounded-xl p-3"><p className="text-xl font-bold text-blue-700">${precioAvg}</p><p className="text-xs text-gray-500 mt-0.5">Promedio</p></div>
            <div className="bg-red-50 rounded-xl p-3"><p className="text-xl font-bold text-red-700">${precioMax}</p><p className="text-xs text-gray-500 mt-0.5">Máximo</p></div>
          </div>
          ...
          {precioRows.length > 0 && (
            <div className="mt-4">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs border-collapse">
                  <thead><tr className="border-b border-gray-100"><th className="text-left py-1.5 pr-3 text-gray-400 font-medium">Fixer</th><th className="text-left py-1.5 pr-3 text-gray-400 font-medium">Comercio</th><th className="text-left py-1.5 pr-3 text-gray-400 font-medium">Precio</th><th className="text-left py-1.5 text-gray-400 font-medium">Fecha</th></tr></thead>
                  <tbody>
                    {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
