...
import { aceptarInvitacionRepo, rechazarInvitacionRepo } from './actions'
...
export function AceptarInvitacionRepoForm({ tokenId, marcaId, repoId, marcaNombre, repoNombre }: Props) {
  ...
  const handleAceptar = () => {
    ...
    setError(null)
    startAceptar(async () => {
      const res = await aceptarInvitacionRepo(tokenId, marcaId, repoId)
      if (res.error) {
        setError(res.error)
      } else {
        router.push('/repositora/dashboard?vinculado=1')
      }
    })
  }
  ...
  const handleRechazar = () => {
    startRechazar(async () => {
      await rechazarInvitacionRepo(tokenId)
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 max-w-md w-full">
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-6">
          ...
          <p className="text-xs text-red-600">
            No te vincularás con {marcaNombre}. Podés pedirles un nuevo link si cambiás de opinión.
          </p>
        </div>
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8 max-w-md w-full">
      <div className="text-center mb-6">
        ...
        <p className="text-sm text-gray-500">
          La marca <strong className="text-gray-700">{marcaNombre}</strong> te invita a establecer una relación comercial con{' '}
      ...
      <div className="bg-gray-50 rounded-xl p-4 mb-6 text-xs text-gray-600 space-y-1">
        ...
        <p>• Gestionar asignaciones de fixers</p>
      </div>
